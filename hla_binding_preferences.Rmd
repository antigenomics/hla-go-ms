...
#human_prots.a <- human_prots
#names(human_prots.a) <- str_split_fixed(names(human_prots), "[\\| ]", 4)[,2]
#human_prots.a2 <- human_prots
#names(human_prots.a2) <- str_split_fixed(names(human_prots), "GN=", 4)[,2]
#names(human_prots.a2) <- str_split_fixed(names(human_prots.a2), " ", 4)[,1]
...
hla.anchor.res <- tibble(hla = c(rep("HLA-A0201", 2), "HLA-A1101", "HLA-B0702", "HLA-B2705", rep("HLA-C0202", 4), rep("HLA-C1507", 3)), aminoacid = c("L", "V", "K", "P", "R", "F", "Y", "A", "L", "L", "A", "V"), anchor = 1)
...
pfig3b <- go.enr.1 %>%
  ...
  tableGrob(rows = NULL, theme = ttheme_default(
    core = list(fg_params=list(cex = .7),
                padding=unit.c(unit(2, "mm"), unit(2, "mm"))),
...
figure <- multi_panel_figure(
  columns = 8,
  rows = 12)
(figure %<>% fill_panel(pfig4a, row = 1:5, column = 1:8))
(figure %<>% fill_panel(pfig4b, row = 6:12, column = 1:8))
dev.off()
```
...
ggdraw() +
  ...
  draw_plot(pst3c, x = .3, y = .21, width = 1, height = 1) 
```

### Explore protein length bias for 93 HLA alleles

```{r}
data.pred.sel.s.all <- fread("data/genes_enr_extended.tsv")
...
(figure %<>% fill_panel(pfig5a, column = 1:7, row = 1:5))
(figure %<>% fill_panel(pfig5b, column = 8:11, row = 1:5))
(figure %<>% fill_panel(pfig5c, column = 1:11, row = 6:8))
dev.off()
```
...
pdf("figures/FigS8.pdf", height = 10, width = 8)
pfigs8
dev.off()
```
...
figure <- multi_panel_figure(
  columns = 11,
  rows = 13)
(figure %<>% fill_panel(pfig6a, row = 1:13, column = 1:5))
(figure %<>% fill_panel(pfig6b, row = 5:8-4, column = 6:8))
(figure %<>% fill_panel(pfig6c, row = 5:8-4, column = 9:11))
(figure %<>% fill_panel(pfig6d, row = 9:13-4, column = 6:11))
(figure %<>% fill_panel(pfig6e, row = 1:4+(13-4), column = 6:11))
dev.off()
```
...
haplotypes.f.control <- c(haplotypes.f.a$MHC.1, haplotypes.f.a$MHC.2) %>% 
  unique() %>% 
...
haplotypes.f.dist <- haplotypes.f.a %>% 
  bind_rows(haplotypes.f.control) %>% 
...
pfig7 <- haplotypes.f.dist %>% 
  ggplot(aes(x = haplotype, y = dist.l)) +
  facet_wrap(~genes) +
  #geom_violin(alpha=.2, color='black', fill = 'grey') + 
  #geom_jitter(size=.5)+
